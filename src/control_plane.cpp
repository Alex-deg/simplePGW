...
std::shared_ptr<pdn_connection> control_plane::create_pdn_connection(
    ...
    if (apn_it == _apns.end()) {
        return nullptr; // APN не существует
    }
    ...
    boost::asio::ip::address_v4 ue_ip(boost::asio::ip::address_v4::bytes_type{
        ...
        static_cast<uint8_t>((next_ip >> 16) & 0xFF),
        static_cast<uint8_t>((next_ip >> 8) & 0xFF),
...
void control_plane::delete_pdn_connection(uint32_t cp_teid) {
    auto pdn = find_pdn_by_cp_teid(cp_teid);
    if (pdn == nullptr) return;

    // Удаляем все bearers этого соединения
...
std::shared_ptr<bearer> control_plane::create_bearer(
    ...
    static uint32_t next_teid = 1;
    uint32_t dp_teid = next_teid++;
